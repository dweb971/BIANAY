    ...
    public function ajouter_user(array $data)
    {
        ...
        if(!isset($_SESSION["login"]))
        {
            ...
            if ($idprofile < 0) {
                ...
                //insert into user
                $reqU = "INSERT INTO user(idprofil,login,password,dateAdd,dateUpdate) VALUE ('".$idprofil."','".$this->get_pass."','".$this->get_pass()."','".$dates."','".$dates."')";
              
                $dbh = $this->get_connectDB()->query($reqU);
              
                if ($dbh->rowCount() === 1) {
                    //insert ok
                    $_SESSION["msg"] = "<p>L'ajout user $idprofil est réussit.</p>";
